...
/**
 * Class MemcacheTags — прокладка для Memcache с тегами и блокировками
...
class MemcacheTags extends Memcache {
	...
	public function get($key, $valid_required = true, $lock_time = 0) {
		...
		if ($is_valid && !empty($data['tags'])) {
			...
			$is_valid = (!(bool)array_udiff_assoc($data['tags'], $this->getTags(array_keys($data['tags'])), function ($a, $b) {
				return intval($a < $b);
			}));
		}
	...
	public function set($key, $data, array $tags = [], $expire = 0) {
		$tags[] = $this->prefix;
		$data = ['data' => $data, 'tags' => $this->getTags($tags)];
	...
	public function delete($key) {
		if (!is_array($key)) return parent::delete($this->prefix.'_'.$key);
	...
	public function increment($key, $data = 1) {
		if (!is_array($key)) return parent::increment($this->prefix.'_'.$key, $data);
	...
	public function decrement($key, $data = 1) {
		if (is_array($key)) return parent::decrement($this->prefix.'_'.$key, $data);
